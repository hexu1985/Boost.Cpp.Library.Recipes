...
void signal_handler(asio::signal_set& signals, const system::error_code& ec, int signo, int i) {
    if (ec) {
    ...
    if (i < 3) {
        signals.async_wait([&signals, i](const system::error_code& ec, int signo) {
