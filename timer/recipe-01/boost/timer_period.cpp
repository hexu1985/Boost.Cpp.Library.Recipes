...
void periodic_task(steady_timer& timer, const boost::system::error_code& /*e*/) {
    print_message("周期性任务执行");
    // 再次设置定时器以实现周期性执行
    timer.expires_after(std::chrono::seconds(2));
    // also can use
    // timer.expires_at(timer.expiry() + std::chrono::seconds(2));
    timer.async_wait([&timer](const boost::system::error_code& e) { periodic_task(timer, e); });
}
...
int main() {
    ...
    timer2.async_wait([](const boost::system::error_code& e) { print_message("10秒过去了"); });
